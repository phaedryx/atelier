    ...
    final class Server: @unchecked Sendable {
        ...
        func stop() {
            queue.async { [weak self] in
                ...
                try? FileManager.default.removeItem(at: Endpoint.fileURL)
            }
        }
        ...
        private func writeEndpointFile(port: UInt16) {
            ...
            do {
                try FilePersistence.writeAtomically(JSONEncoder().encode(endpoint), to: Endpoint.fileURL)
        ...
        private func receive(on connection: NWConnection, buffer: Data) {
            connection.receive(minimumIncompleteLength: 1, maximumLength: 65_536) { [weak self] data, _, isComplete, error in
                ...
                let (lines, remainder) = Framing.lines(from: accumulated)
                for line in lines {
        ...
        private func send(_ response: Response, on connection: NWConnection) {
            guard let data = try? Framing.encode(response) else {
