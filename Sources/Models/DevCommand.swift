...
extension DevCommand {
    enum Resolver {
        ...
        /// load `compose.yaml` — a name Atelier does not detect — while bootstrap
        /// and dispose ran something else for the same project.
        ///
        /// **The `command` built here carries no `-n`, so executing it would run
        /// *every* namespace the config declares — `bootstrap` and `dispose`
        /// included.** Those two are the phases `PhasePolicy` gates: they run the
        /// project's processes unattended, with nobody watching. This string never
        /// goes through `PhasePolicy`, so it must never reach a shell.
        ///
