...
/// Claude plan-usage reporting: probing for it, the shape that comes back,
...
extension Usage {
    enum Probe {
        ...
        static func parse(_ data: Data) -> Usage.Report? {
        ...
        static func parseText(_ text: String) -> Usage.Report? {
            var report = Usage.Report()
            for rawLine in text.split(whereSeparator: { $0 == "\n" || $0 == "\r" }) {
                let line = String(rawLine)
                let lower = line.lowercased()
                guard lower.contains("% used"), let percent = firstPercent(in: line) else { continue }
                let window = Usage.Report.Window(percentUsed: percent, resetText: resetText(in: line))
