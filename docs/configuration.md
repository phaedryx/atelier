...
This is the user-facing reference. The decisions behind these files, and the
implementation notes, are in
[docs/agents/execution-and-config.md](agents/execution-and-config.md) — change
one and check the other.

Atelier runs a project's commands through
[process-compose](https://f1bonacc1.github.io/process-compose/), so it needs that
binary on the machine. It is a requirement rather than an option: there is no
switch to turn it on, and nothing below works without it, including the Execution
tab's Start button.

The Verification tab is the exception: its checks are plain commands, declared in
their own file and run without process-compose. See [Verification](#verification)
below.
...
| File | What it holds |
|------|---------------|
| `execution.process-compose.yaml` | the commands, in three namespaces |
| `initialization.yaml` | the steps run once, when a worktree is created |
| `verification.yaml` | the checks the Verification tab runs |
| `ports.yaml` (or `ports.yml`) | the port variables Atelier supplies |

All four are read **from the project directory only**.
...
`execution.process-compose.yaml` is process-compose's own format; the rest are
Atelier's. **A new project starts with a commented template of the execution and
verification files**, written by New Project and Clone Repository. Adding a
directory you already have leaves it alone.

**One name, one place.** The lookup is `execution.process-compose.yaml` in the
project directory, then `execution.process-compose.yml`, and that is the whole of
it — nothing inside a worktree is read. That is a trust decision rather than a
...
Worktree setup is the other file — see [Initialization](#initialization) for what
this project's `initialization.yaml` holds. Everything from here on is
...
processes:
  ...
  preflight:
    ...
    command: |
      ...
      # without Atelier. Under Atelier the names ports.yaml declares are always
      # set, so a fallback that fires means ports.yaml misspelled or forgot one —
      ...
      # Add a line here for each port ports.yaml gains, or the new one is exactly
      # the case this guard was written to catch.
      if [ -n "$${ATELIER_WORKTREE_DIR:-}" ]; then
        : "$${BFF_PORT:?not declared in ports.yaml}"
        : "$${RAILS_PORT:?not declared in ports.yaml}"
        : "$${HTML_TO_JSON_PORT:?not declared in ports.yaml}"
        : "$${VITE_PORT:?not declared in ports.yaml}"
      fi
    availability:
      restart: "no"
  ...
  #
  # `${NAME:-default}` throughout: Atelier sets every name ports.yaml declares,
...
**Why the `:-3006` defaults are there.** `ports.yaml` carries no number for an
...
the picture. Once the ports come out of the repo's own env files and live only
here, that fallback is the last remaining record of the stack's conventional
layout, which is a reason to keep it rather than inline the numbers or drop them.

What it cannot do is tell "no Atelier" from "declared it wrong". A name
`ports.yaml` misspells — or never declares — still yields a running stack, on the
...
command and in no plain shell. Keep the defaults; make them prove they were
unnecessary.
...
daemons. `prepare` is the right place to check for those — a stack that fails on
a missing daemon should say so once, before five processes each fail differently.

## Finding process-compose
...
have arrived with a clone. Rule of thumb — if a file can arrive with a clone,
