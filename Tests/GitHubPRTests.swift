// ABOUTME: Tests for GitHubPR JSON decoding, check rollup reduction, and status mapping.
...
final class GitHubPRTests: XCTestCase {
    ...
    func testDecodesCoreFields() {
        let prs = GitHubPR.decode(listJSON(pr(number: 42)))
    ...
    func testMalformedJSONDecodesToEmpty() {
        XCTAssertTrue(GitHubPR.decode(Data("not json".utf8)).isEmpty)
        XCTAssertTrue(GitHubPR.decode(Data()).isEmpty)
    }
    ...
    func testEmptyReviewDecisionNormalizesToNil() {
        let prs = GitHubPR.decode(listJSON(pr(reviewDecision: "")))
        XCTAssertNil(prs[0].reviewDecision)
    }

    func testReviewDecisionPreserved() {
        let prs = GitHubPR.decode(listJSON(pr(reviewDecision: "CHANGES_REQUESTED")))
    ...
    func testEmptyRollupIsNone() {
        let prs = GitHubPR.decode(listJSON(pr(rollup: "[]")))
        XCTAssertEqual(prs[0].checks, .none)
    }

    func testAllSuccessfulCheckRunsArePassing() {
        let rollup = "[\(checkRun(status: "COMPLETED", conclusion: "SUCCESS"))]"
        XCTAssertEqual(GitHubPR.decode(listJSON(pr(rollup: rollup)))[0].checks, .passing)
    }
    ...
    func testFailedCheckRunIsFailing() {
        ...
        """
        XCTAssertEqual(GitHubPR.decode(listJSON(pr(rollup: rollup)))[0].checks, .failing)
    }
    ...
    func testInProgressCheckRunIsPending() {
        ...
        """
        XCTAssertEqual(GitHubPR.decode(listJSON(pr(rollup: rollup)))[0].checks, .pending)
    }
    ...
    func testTimedOutAndActionRequiredCountAsFailing() {
        for conclusion in ["TIMED_OUT", "ACTION_REQUIRED", "STARTUP_FAILURE"] {
            ...
            XCTAssertEqual(
                GitHubPR.decode(listJSON(pr(rollup: rollup)))[0].checks,
                .failing,
    ...
    func testOnlyNonFailingNonSuccessConclusionsIsNone() {
        ...
        XCTAssertEqual(GitHubPR.decode(listJSON(pr(rollup: rollup)))[0].checks, .none)
    }
    ...
    func testDraftOutranksOpen() {
        let prs = GitHubPR.decode(listJSON(pr(state: "OPEN", isDraft: true)))
        XCTAssertEqual(prs[0].status, .draft)
    }

    func testOpenStatus() {
        XCTAssertEqual(GitHubPR.decode(listJSON(pr(state: "OPEN")))[0].status, .open)
    }

    func testMergedStatus() {
        XCTAssertEqual(GitHubPR.decode(listJSON(pr(state: "MERGED")))[0].status, .merged)
    }

    func testClosedUnmergedStatus() {
        XCTAssertEqual(GitHubPR.decode(listJSON(pr(state: "CLOSED")))[0].status, .closed)
    }

    /// A merged PR is never a draft, whatever the flag says.
    func testMergedIsNeverDraft() {
        let prs = GitHubPR.decode(listJSON(pr(state: "MERGED", isDraft: true)))
        XCTAssertEqual(prs[0].status, .merged)
    }

    func testUnknownStateFallsBackToClosed() {
        XCTAssertEqual(GitHubPR.decode(listJSON(pr(state: "BOGUS")))[0].status, .closed)
    }
    ...
    func testOpenBeatsMergedForSameBranch() {
        ...
        let prs = GitHubPR.decode(Data("[\(merged),\(open)]".utf8))
        XCTAssertEqual(GitHubPR.byBranch(prs)["shared"]?.number, 3)
    }
    ...
    func testMergedBeatsClosedForSameBranch() {
        ...
        let prs = GitHubPR.decode(Data("[\(closed),\(merged)]".utf8))
        XCTAssertEqual(GitHubPR.byBranch(prs)["shared"]?.number, 4)
    }
    ...
    func testHighestNumberWinsWithinSameStatus() {
        ...
        let prs = GitHubPR.decode(Data("[\(a),\(b)]".utf8))
        XCTAssertEqual(GitHubPR.byBranch(prs)["shared"]?.number, 20)
    }

    func testDistinctBranchesAreAllKept() {
        let prs = GitHubPR.decode(listJSON(pr(number: 1), pr(number: 2)))
        let byBranch = GitHubPR.byBranch(prs)
